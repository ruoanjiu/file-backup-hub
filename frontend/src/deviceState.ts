...
export function deviceAuthorizationLabel(device: DeviceAuthorizationState): string {
  ...
  return device.enabled === false ? '已停用（可移除）' : '已配对'
}

export function removeDeviceConfirmation(device: DeviceAuthorizationState): string {
  const name = device.display_name || device.device_id || '该设备'
  return `确认从设备列表移除“${name}”吗？\n\n只会删除这条已停用的设备登记。历史备份、Manifest、Transfer、Trash 和 Client 原始文件都不会删除。`
}
