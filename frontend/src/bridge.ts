...
const mockClient = {
  ...
  servers: [
    { id: 'server-a', name: 'Server A', status: 'ok', usage: '2.35 TB / 4 TB', percent: 58 },
    { id: 'server-b', name: 'Server B', status: 'ok', usage: '1.62 TB / 4 TB', percent: 40 },
  ],
  devices: [
    ...
    { device_id: 'office-pc-a', display_name: '办公室电脑 A', status: 'offline', paired: true },
  ],
  tasks: [
...
const mockServer = {
  mode: 'server',
  server: { id: 'server-a', status: 'ok', endpoint: 'http://192.168.1.100:8000', data_dir: '/Users/Shared/FileBackupServer' },
  ...
  inbox: [
    { transfer_id: 'transfer_server_demo', sender_device_id: 'office-pc', receiver_device_id: '__server__', status: 'AVAILABLE', file_count: 2, total_size: 4096 },
  ],
}
...
export async function bridge<T = unknown>(method: string, ...args: unknown[]): Promise<T> {
  ...
  if (method === 'choose_server_data_dir') return ['/Users/Shared/FileBackupServer'] as T
  if (method === 'save_server_settings') return { status: 'UPDATED', restarted: true } as T
  if (method === 'revoke_device') return { status: 'REVOKED', backups_deleted: false } as T
  if (method === 'remove_device') return { status: 'REMOVED', device_record_only: true, backups_deleted: false } as T
  if (method === 'disconnect_server') return { status: 'DISCONNECTED', remaining_servers: 1, backups_deleted: false } as T
  if (method === 'delete_server') return { status: 'DELETED_LOCAL_CONFIG', remaining_servers: 0, remote_data_deleted: false } as T
  if (method === 'add_server') return { status: 'SUCCESS', server_id: 'server-new' } as T
  if (method === 'reject_transfer' || method === 'reject_server_transfer') return { status: 'REJECTED', source_files_deleted: false } as T
  if (method === 'receive_server_transfer') return { status: 'COMPLETED', received_count: 2, destination: '/Users/Shared/FileBackupServer/transfers/server-inbox/demo' } as T
