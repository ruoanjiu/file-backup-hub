...
test('device status distinguishes active, revoked and legacy authorization', () => {
  ...
  assert.equal(deviceAuthorizationLabel({ paired: true, enabled: false }), '已停用（可移除）')
...
test('confirmation copy explicitly preserves backup data and other servers', () => {
  ...
  const removeDevice = removeDeviceConfirmation({ display_name: '办公室电脑' })
  ...
  assert.match(removeDevice, /只会删除这条已停用的设备登记/)
  assert.match(removeDevice, /历史备份、Manifest、Transfer、Trash/)
})
