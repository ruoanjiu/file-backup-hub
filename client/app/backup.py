...
def prepare_backup(
    ...
    scanned = scan_strategy_files(strategy, config.backup)
    if not scanned:
        raise ValueError(f"No files matched strategy {strategy.name}")

    manifest_files = copy_files_to_workdir(scanned, workdir)
    manifest = build_manifest(config, strategy, backup_id, created_at, manifest_files)
...
def run_backup_for_strategy(
    ...
    db.start_backup_job(backup_id, config.client.machine_id, strategy.name, created_at.isoformat())

    prepared: PreparedBackup | None = None
    try:
        prepared = prepare_backup_with_id(config, strategy, backup_id, created_at)
